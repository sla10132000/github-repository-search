...
Kent C. Dodds の**テストトロフィー**の考え方に基づき、Integration テストを最も厚く設計している。

```
          /  E2E  \              ← 1シナリオ（クリティカルパス）
         /----------\
        / Integration \          ← コンポーネント描画・操作（最も厚い）
       /--------------\
...
- **Integration**: コンポーネントの描画・ユーザー操作・URL 遷移（6ファイル）
- **E2E**: 検索 → 結果表示 → 詳細遷移 → 戻る のクリティカルパスのみ

#### Unit テストが残っている理由

テストトロフィーは「Unit テスト不要」ではなく、**「Integration で書けるものは Integration で書け」** という考え方。Unit テストは以下の2つに絞っている。

1. **UIが存在しない純粋ロジック** — `calcVisiblePages`（ページ計算）や型ガード（`isValidRepository` 等）はユーザー操作の概念がなく、Integration テストの対象にならない
2. **Integration レベルで再現できないエラー** — 403/404/422/ネットワーク断は実際のコンポーネントを動かして意図的に起こせないため、`fetch` をモックして直接検証する必要がある
...
- **テスト方針の決定**: テストトロフィー採用・各テストの選定理由など、何をどう検証するかの判断は自分で行った
- **動作確認（PC）**: 実際にブラウザで操作し、挙動・表示を目視確認した
- **スマホ動作確認**: 実機でのタッチ操作・レイアウト崩れ・使いやすさを確認した
- **GitHub API レート制限の挙動確認**: 実際にレート制限が発動したときのエラー表示・メッセージが適切かを確認した
- **UX の判断**: 検索結果の見やすさ・操作感など、使いやすいかどうかの感覚的な判断は自分で行った
